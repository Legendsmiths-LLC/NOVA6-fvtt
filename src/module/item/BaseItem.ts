export abstract class BaseItem {
    ...
    static activateActorSheetListeners(html, sheet) {
        ...
        html.find(`.nova6-js-${this.documentName}-settings`).click((e) => this._onItemSettings.call(this, e, sheet));
        html.find(`.nova6-js-${this.documentName}-delete`).click((e) => this._onItemDelete.call(this, e, sheet));
    }
    ...
    static _onItemDelete(e, sheet) {
        ...
        new Dialog(
            {
                title: `${game.i18n.localize("NOVA.Dialog.DocumentDelete")} ${item.name}`,
                content: game.i18n.localize("NOVA.Dialog.DocumentDeleteText"),
                default: "submit",
                buttons: {
                    cancel: {
                        icon: '<i class="fas fa-times"></i>',
                        label: game.i18n.localize("NOVA.Dialog.Cancel"),
                        callback: () => null,
                    },
                    submit: {
                        icon: '<i class="fas fa-check"></i>',
                        label: game.i18n.localize("NOVA.Dialog.Confirm"),
                        callback: async () => {
                            item.delete();
                        },
                    },
    ...
    protected static isEditMode(e): boolean {
        ...
        return !!element.closest(".nova6-js-edit-mode").length;
    }
}
